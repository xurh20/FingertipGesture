...
if __name__ == '__main__':
    ...
    parser.add_argument(
        ...
        '--amplitude',
        default=3,
        help='specify the amplitude you want to record, default as 3')
    args = parser.parse_args()

    idx = 0
    while True:
        save_dir = "new_data/am_" + str(int(args.amplitude)) + '_' + str(idx)
    ...
    if handle:
        ...
        s.sendto(repr(int(args.amplitude)).encode('gbk'), ('localhost', 34826))
        candidates = list(
            itertools.product(list(range(4)), list(range(int(args.amplitude))),
                              list(range(5))))
        ...
        while True:
            if plotting:
                ...
                s.sendto(
                    repr(target[0] * int(args.amplitude) +
                         target[1]).encode('gbk'), ('localhost', 34826))
