            ...
            for word in words:
                ...
                if (len(newWord) > 2):
                    for j in range(1, len(newWord) - 1):
                        lastVector = STD_KB_POS[lowerCase(newWord[j])] - STD_KB_POS[lowerCase(newWord[j - 1])]
                        nowVector = STD_KB_POS[lowerCase(newWord[j + 1])] - STD_KB_POS[lowerCase(newWord[j])]
                        if (lastVector[1] == 0 and nowVector[1] == 0 and lastVector[0] * nowVector[0] > 0):
                            continue
                        else:
                            newWord_1 += newWord[j]
