...
def linear_rectangle(pos):
    ...
    width = 8
    height = 8
    return np.array([center[0] + pos[0] * width / STD_KB_WIDTH,
...
def genPattern(sentence, word):
    word = allPattern[sentence - 1][word]
    if (len(word) == 1):
        print("error genpattern with only one word")
    patterns = []
    for i in range(len(word) - 1):
        v = linear_rectangle(STD_KB_POS[lowerCase(word[i + 1])]) - linear_rectangle(STD_KB_POS[lowerCase(word[i])])
        # print(v)
        v = v / np.linalg.norm(v)
        patterns.append(v)
    # print(word)
...
def showDistPath(sentence, word, person):
    ...
    print(vectors)
    print(patterns)
    d, cost_matrix, acc_cost_matrix, path = dtw(vectors, patterns, dist=distance)
    print(d)
    print(path)
...
def genLabelPoints(sentence, word, person):
    ...
    if (len(vectors) == 0 or len(patterns) == 0):
        ...
        return []
    d, cost_matrix, acc_cost_matrix, path = a_dtw(vectors, patterns, dist=distance)
    # print(path)
        ...
        for i in range(1, 82):
            for j in range(len(allPattern[i - 1])):
                if os.path.exists(BASE_DIR + person + "/" + str(i) + "_" + str(j) + ".npy"):
                    with open(SAVE_MP_DIR + person + "_" + str(i) + "_" + str(j) + ".txt", "w") as f:
                        f.write(json.dumps(np.array(genLabelPoints(i, j, person)).tolist()))
                else:
